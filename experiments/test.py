...
def parse_args():
    ...
    parser.add_argument("--adversary", type=str, default="ddpg", help="policy used by adversary")
    parser.add_argument("--normal", type=str, default="ddpg", help="policy used by normal agent")
...
def test(arglist):
    with U.single_threaded_session():
        ...
        obs_shape_n = [[env.observation_space[i].shape for i in range(env.n)] for idx in range(num_env)]
        num_adversaries = min(env.n, arglist.num_adversaries)
        names = ['ddpg', 'maddpg', 'mmmaddpg']
        names2id = { names[i]:i for i in range(num_env)}
        ...
        while True:
            ...
            if len(episode_rewards) >= arglist.num_episodes:
                ...
                agrew_file_name = arglist.plots_dir + arglist.exp_name + '_agrewards_test.pkl'
